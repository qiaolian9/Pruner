...
def get_network(network_args):
    ...
    if name in ['resnet_18', 'resnet_50', 'mobilenet_v2', 'mobilenet_v3',
        ...
        network_key = (name, [(batch_size, 3, 256, 256)])
    elif name == 'detr':
        network_key = (name, [(batch_size, 3, 256, 256)])
    else:
...
def get_tuning_option(tuning_args, target, num_tasks=None):
    ...
    if "cpu" in target.keys:
        ...
        tuning_opt = auto_scheduler.TuningOptions(
            num_measures_per_round=num_measures_per_round,
            num_measure_trials=n_trials if num_tasks == None else n_trials * num_tasks,
...
def tune_and_evaluate(network_args, tuning_args, target, target_host, result_file, transfer_tune, search_type):
    ...
    if not tuning_args['eval_only']:
        ...
        # tuning_opt, measure_ctx = get_tuning_option(tuning_args, target, len(tasks))

        # Run search
        tuner = auto_scheduler.TaskScheduler(tasks, task_weights,
            load_model_file=tuning_args['load_model'], load_log_file=tuning_args['log_file'])
        # tuner = auto_scheduler.TaskScheduler(tasks, task_weights,
        #     load_model_file=tuning_args['load_model'], load_log_file=tuning_args['log_file'], strategy="round-robin")
        policy = 'sketch.%s' % (tuning_args['cost_model'])
        print(policy, tuning_args['psa_model_type'])
        if not transfer_tune:
            tuner.tune(tuning_opt, search_policy=policy, psa_model_type=tuning_args['psa_model_type'])
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--psa", type=str, default='a100_40')
    ...
    parser.add_argument("--cost-model", type=str, choices=['xgb', 'lgbm', 'random', 'xgb-no-update', 'lgbm-no-update', 'mlp', 'mlp-no-update',
                      'tab', 'tab-no-update', 'pam', 'pam-no-update', 'pam-siamese-update'],
    ...
    parser.add_argument("--num-measure-per-iter", type=int, default=10,
    ...
    tuning_args = {
        ...
        "num_measures_per_round": args.num_measure_per_iter,
        "log_file": log_file,
        "run_timeout": args.run_timeout,
        "cost_model": args.cost_model,
        "load_model": args.load_model,
        "n_lines": args.n_lines,
        'psa_model_type': args.psa
    }
