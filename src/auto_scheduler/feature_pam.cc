...
#include <tvm/auto_scheduler/feature_pam.h>
...
template <class T>
using BufferNameMap = std::unordered_map<Buffer, T, ObjectHash, ObjectEqual>;

using BufferCache = std::unordered_map<Buffer, std::vector<Buffer>, ObjectHash, ObjectEqual>;

// The number of samples to extract for arithmetic intensity curves
static const int ARITH_INTENSITY_CURVE_SAMPLE_N = 10;

static const int PAM_FEATURE_SIZE = 23;
static const int Ansor_FEATURE_SIZE = 164;


// Annotation position encoding
enum class AnnotationPosTypePAM : int {
...
// Buffer access type
enum class BufferAccessTypePAM : int { kRead = 0, kWrite = 1, kReadWrite = 2, kUnknownRW = 3 };
// Data location type
enum class LocationTypePAM : int { globalMem = 0, sharedMem = 1, localMem = 2 };

// Accesses to a buffer
struct BufferAccessPAM {
  // data reuse type
  BufferAccessTypePAM acc_type{BufferAccessTypePAM::kUnknownRW};
...
// Data reuse type
enum class ReuseTypePAM : int { kLoopMultipleRead = 0, kSerialMultipleReadWrite = 1, kNoReuse = 2 };

// Feature for an access of a buffer
struct BufferAccessFeaturePAM {
  std::string buffer_name;        // The name of the buffer
  BufferAccessTypePAM acc_type;      // The type of the access
  LocationTypePAM buf_loc;           // buffer location
  ...
  ReuseTypePAM reuse_type;           // Tye type of data reuse
...
struct FeatureSetPAM {
  ...
  AnnotationPosTypePAM vec_type;       // The type of vectorization position
  ...
  AnnotationPosTypePAM unroll_type;    // The type of unroll position
  ...
  AnnotationPosTypePAM parallel_type;  // The type of parallel position
  ...
  std::vector<BufferAccessFeaturePAM> access_feas;
...
struct BufferFeatureSetPAM {
  // Group 1: Computation related features
  float float_op;                  // The number of float MAD (Multiply–add) ops

  // Group 2: Buffer access related features (per buffer)
  BufferAccessFeaturePAM access_fea;
  float alloc_size{0};        // The size of allocated buffer in bytes
  
};

// Return whether a var is in an expr
bool VarInExprPAM(const Var& var, const PrimExpr& expr) {
  bool find = false;
...
AnnotationPosTypePAM GetAnnotationPosEncodingPAM(const Var& var, const Array<PrimExpr>& spatial_args,
  ...
  for (size_t i = 0; i < spatial_args.size(); ++i) {
    if (VarInExprPAM(var, spatial_args[i])) {
      find_i = i;
      find_ct += 1;
    }
  }
  ...
  if (find_ct == 0) {
    ...
    if (find_ct >= 1) {
      if (find_i == 0) {
        return AnnotationPosTypePAM::kPosInnerReduce;
      } else if (find_i == reduce_axis.size() - 1) {
        return AnnotationPosTypePAM::kPosOuterReduce;
      } else {
        return AnnotationPosTypePAM::kPosMiddleReduce;
      }
    } else {
      ...
      return AnnotationPosTypePAM::kPosOuterSpatial;
    }
  } else if (find_ct == 1) {
    if (find_i == spatial_args.size() - 1) {
      return AnnotationPosTypePAM::kPosInnerSpatial;
    } else if (find_i == 0) {
      return AnnotationPosTypePAM::kPosOuterSpatial;
    } else {
      return AnnotationPosTypePAM::kPosMiddleSpatial;
    }
  } else {
    return AnnotationPosTypePAM::kPosMixed;
  }
}

// Return the extent of a for loop
int64_t GetLoopExtentPAM(const ForNode* node) {
...
class MathOpCounterPAM : public StmtExprVisitor {
 public:
...
class BufferAccessExtractorPAM : public StmtExprVisitor {
 public:
  void ExtractReads(const PrimExpr& expr) { this->VisitExpr(expr); }

  void InsertAccess(const Buffer& buf, BufferAccessTypePAM acc_type, const Array<PrimExpr>& indices) {
    BufferAccessPAM& acc = buf_accesses[buf];
    acc.acc_type = acc_type;
    acc.indices.push_back(std::vector<PrimExpr>(indices.begin(), indices.end()));
  }

  void VisitExpr_(const BufferLoadNode* op) final {
    BufferAccessPAM& acc = buf_accesses[op->buffer];
    switch (acc.acc_type) {
      case BufferAccessTypePAM::kRead:
        break;
      case BufferAccessTypePAM::kWrite:
        acc.acc_type = BufferAccessTypePAM::kReadWrite;
        break;
      case BufferAccessTypePAM::kReadWrite:
        break;
      case BufferAccessTypePAM::kUnknownRW:
      default:
        acc.acc_type = BufferAccessTypePAM::kRead;
        break;
    }

    if (acc.acc_type != BufferAccessTypePAM::kReadWrite) {
  ...
  BufferMap<BufferAccessPAM> buf_accesses;
};
...
class CoefficientExtractorPAM : public StmtExprVisitor {
 public:
...
int64_t ComputeStridePAM(const std::vector<std::vector<PrimExpr>>& indices,
                      const std::vector<int>& shape, const VarNode* stride_var) {
  int64_t min_stride = std::numeric_limits<int64_t>::max();
  bool find = false;
  CoefficientExtractorPAM extractor;
...
int KMP(std::string mem_pattern) {
  std::string needle = "20101112";
  int next[needle.size()];
  getNext(next, needle);
  size_t j = 0;
  for (size_t i = 0; i < mem_pattern.size(); i++) {
      while(j > 0 && mem_pattern[i] != needle[j]) {
          j = next[j - 1];
      }
      if (mem_pattern[i] == needle[j]) {
          j++;
      }
      if (j == needle.size() ) {
...
void ComputeRegionPAM(const std::vector<std::vector<PrimExpr>>& indices, arith::Analyzer* ana,
...
std::tuple<ReuseTypePAM, float, float, float> ComputeReusePAM(
    ...
    const std::unordered_map<const ForNode*,
                             BufferMap<std::vector<std::tuple<BufferAccessTypePAM, int64_t, int>>>>&
        for_touch_regions) {
  ...
  for (int i = static_cast<int>(for_loop_stack.size()) - 1; i >= 0; --i) {
    ...
    int64_t extent = GetLoopExtentPAM(for_loop_stack[i]);
    bool find = false;

    for (size_t j = 0; j < indices.size(); j++) {
      for (size_t k = 0; k < indices[j].size(); k++) {
        if (VarInExprPAM(cur_for->loop_var, indices[j][k])) {
          find = true;
          break;
        }
      }
    ...
    if (scan_status == 0) {  // accumulate reuse distance
      ...
      if (find) {
        return std::make_tuple(ReuseTypePAM::kLoopMultipleRead, reuse_dis_iter, reuse_dis_bytes, reuse_ct);
      } else {
        reuse_ct *= extent;
      }
    }

    const BufferMap<std::vector<std::tuple<BufferAccessTypePAM, int64_t, int>>>& buffer_map =
        for_touch_regions.at(cur_for);

    int serial_reuse = static_cast<int>(buffer_map.at(buf).size()) - 1;
    if (serial_reuse > 0) {
      // Have SerialMultipleReadWrite reuse
      int64_t cur_extent = GetLoopExtentPAM(cur_for);
      ...
      for (const auto& iter : for_touch_regions.at(cur_for)) {
        for (const auto& access : iter.second) {
          ...
          if (std::get<0>(access) == BufferAccessTypePAM::kRead) {
            reuse_dis_bytes += std::get<1>(access) * std::get<2>(access);
          }
        }
      }
      ...
      reuse_ct = 1.0;
      for (int j = i; j >= 0; j--) {
        reuse_ct *= GetLoopExtentPAM(for_loop_stack[j]);
      }
      ...
      return std::make_tuple(ReuseTypePAM::kSerialMultipleReadWrite, reuse_dis_iter / cur_extent,
                             reuse_dis_bytes / cur_extent, reuse_ct);
    }
  }

  if (scan_status == 0) {
    return std::make_tuple(ReuseTypePAM::kNoReuse, 0, 0, 0);
  } else {
    return std::make_tuple(ReuseTypePAM::kLoopMultipleRead, reuse_dis_iter, reuse_dis_bytes, reuse_ct);
  }
}
...
class PerStoreFeatureExtractorPAM : public StmtExprVisitor {
 public:
  explicit PerStoreFeatureExtractorPAM(int cache_line_size) : cache_line_size_(cache_line_size) {}
  ...
  void VisitStmt_(const ForNode* node) final {
    int64_t loop_extent = GetLoopExtentPAM(node);
  ...
  void VisitStmt_(const BufferStoreNode* node) final {
    MathOpCounterPAM math_op_counter;
  ...
  void VisitStmt_(const BufferRealizeNode* node) final {
    ...
    if (!buf_map_.count(key)) {
      ...
      // std::cout << key << "\t" << skey.to_string() << std::endl;
    }
  ...
  void ExtractComputationFeature(const BufferStoreNode* node,
                                 const MathOpCounterPAM& math_op_counter) {
    FeatureSetPAM& fea = buffer_features[node->buffer];
    ...
    fea.vec_type = fea.unroll_type = fea.parallel_type = AnnotationPosTypePAM::kPosNone;

    fea.vec_num = vec_for_stack_.size();
    if (!vec_for_stack_.empty()) {
      fea.vec_len = GetLoopExtentPAM(vec_for_stack_.back());
      fea.vec_prod = 1.0;
      for (const ForNode* pfor : vec_for_stack_) {
        fea.vec_prod *= GetLoopExtentPAM(pfor);
      }
      fea.vec_type = AnnotationPosTypePAM::kPosMixed;
      // todo(merrymercy): this feature requires operation (tvm.compute) information
      // GetAnnotationPosEncodingPAM(vec_for_stack_.back()->loop_var,
      // node->args, pcompute->axis, pcompute->reduce_axis);
    }

    fea.unroll_num = unroll_for_stack_.size();
    if (!unroll_for_stack_.empty()) {
      fea.unroll_len = GetLoopExtentPAM(unroll_for_stack_.back());
      fea.unroll_prod = 1.0;
      for (const ForNode* pfor : unroll_for_stack_) {
        fea.unroll_prod *= GetLoopExtentPAM(pfor);
      }
      fea.unroll_type = AnnotationPosTypePAM::kPosMixed;
      // GetAnnotationPosEncodingPAM(unroll_for_stack_.back()->loop_var,
      // node->args, pcompute->axis, pcompute->reduce_axis);
    }

    fea.parallel_num = parallel_for_stack_.size();
    if (!parallel_for_stack_.empty()) {
      fea.parallel_len = GetLoopExtentPAM(parallel_for_stack_.back());
      fea.parallel_prod = 1.0;
      for (const ForNode* pfor : parallel_for_stack_) {
        fea.parallel_prod *= GetLoopExtentPAM(pfor);
      }
      fea.parallel_type = AnnotationPosTypePAM::kPosMixed;
      // GetAnnotationPosEncodingPAM(parallel_for_stack_.back()->loop_var,
      // node->args, pcompute->axis, pcompute->reduce_axis);
    }
  ...
  void ExtractBufferAccessFeature(const BufferStoreNode* node, const MathOpCounterPAM& math_op_counter,
                                  double* cur_compute_ops, std::vector<float>* compute_ops_list,
                                  std::vector<float>* mem_bytes_list) {
    FeatureSetPAM& fea = buffer_features[node->buffer];

    // Extract all buffer accesses
    std::vector<BufferAccessFeaturePAM> acc_feas;
    BufferAccessExtractorPAM buf_extractor;
    buf_extractor.InsertAccess(node->buffer, BufferAccessTypePAM::kWrite, node->indices);
    ...
    *cur_compute_ops = math_op_counter.float_mad + math_op_counter.float_addsub +
    ...
    for (int i = static_cast<int>(for_loop_stack_.size()) - 1; i >= 0; i--) {
      ...
      BufferMap<std::vector<std::tuple<BufferAccessTypePAM, int64_t, int>>>& buffer_regions_map =
          for_touch_regions_[p_for];

      int64_t mem_bytes = 0;
      for (const auto& x : buf_extractor.buf_accesses) {
        const Buffer& t = x.first;
        const BufferAccessPAM& acc = x.second;

        ComputeRegionPAM(acc.indices, &ana_, &tmp_region);
      ...
      *cur_compute_ops *= GetLoopExtentPAM(for_loop_stack_[i]);
    ...
    for (const auto& x : buf_extractor.buf_accesses) {
      ...
      buffer_times[t] = 1;
      if (buffer_times.count(t) == 0) {
        buffer_times[t] = 1;
      } else {
        buffer_times[t] += 1;
      }
      String new_name = t->name + std::to_string(buffer_times[t]);
      // BufferTime fake_buf = {t, new_name};
      Buffer fake_buf = decl_buffer(t->shape, t->dtype, new_name, t->span);

      buf_seq.push_back(fake_buf);
      // std::cout << "record buf access name\t" << new_name  << "\t" << t->name << std::endl;
      const BufferAccessPAM& acc = x.second;
      fake_buf_set[t].push_back(fake_buf);
      BufferFeatureSetPAM& per_buf_fea = per_buffer_features[fake_buf];
      ...
      if (for_loop_stack_.empty()) {
        ...
        for (i = static_cast<int>(for_loop_stack_.size()) - 1; i >= 0; i--) {
          stride = ComputeStridePAM(acc.indices, int_shape, for_loop_stack_[i]->loop_var.get());
          if (stride != 0) {
            break;
          }
          reduce_ratio *= GetLoopExtentPAM(for_loop_stack_.back());
        }
      ...
      ReuseTypePAM reuse_type;
      float reuse_dis_iter, reuse_dis_bytes, reuse_ct;
      std::tie(reuse_type, reuse_dis_iter, reuse_dis_bytes, reuse_ct) =
          ComputeReusePAM(t, acc.indices, for_loop_stack_, for_touch_regions_);

      

      acc_feas.emplace_back();
      BufferAccessFeaturePAM& acc_fea = acc_feas.back();
      ...
      if (buf_map_.count(t) && StrStartsWith(buf_map_[t], "local")) {
        acc_fea.buf_loc = LocationTypePAM::localMem;
      } else if (buf_map_.count(t) && StrStartsWith(buf_map_[t], "shared")) {
        acc_fea.buf_loc = LocationTypePAM::sharedMem;
      } else {
        acc_fea.buf_loc = LocationTypePAM::globalMem;
      }
      // std::cout << "buf loc\t" << static_cast<int>(acc_fea.buf_loc) << "\t" << t->name << std::endl;
      mem_pattern.append(std::to_string(static_cast<int>(acc_fea.buf_loc)));

      per_buf_fea.access_fea = acc_fea;
      per_buf_fea.float_op = *cur_compute_ops * outer_loop_prod_;
    }
  ...
  void ExtractAllocationFeature(const BufferRealizeNode* node) {
    ...
    for (auto fake_buf : fake_buf_set[node->buffer]) {
      BufferFeatureSetPAM& per_buf_fea = per_buffer_features[fake_buf];
      // allocation feature
      // std::cout << fake_buf->name << "\t" << node->buffer->name << std::endl;
      per_buf_fea.alloc_size = allocation_size * node->buffer->dtype.bytes();
    }
  }
  ...
  // Stores FeatureSetPAM for every buffer
  BufferMap<FeatureSetPAM> buffer_features;
  // Stores FeatureSetPAM for every buffer
  BufferNameMap<BufferFeatureSetPAM> per_buffer_features;
  // Stores buffer exist time
  BufferMap<int> buffer_times;
  std::vector<Buffer> buf_seq;
...
void GetPerStoreFeaturePAM(const Stmt& stmt, int cache_line_size, int max_n_bufs,
                        std::vector<float>* ret) {
  PerStoreFeatureExtractorPAM extractor(cache_line_size);
  extractor(stmt);

  ret->push_back(extractor.buffer_features.size());
  int pattern_index = KMP(extractor.mem_pattern);
  // std::cout << "mem pattern" << extractor.mem_pattern << std::endl;
  ret->push_back(static_cast<float>(pattern_index));

  // feature for write stmt
  for (const auto& x : extractor.buffer_features) {
    const FeatureSetPAM& fea_set = x.second;
    ...
    for (int i = 0; i <= static_cast<int>(AnnotationPosTypePAM::kPosMixed); i++) {
    ...
    for (int idx : buf_order) {
      ...
      for (int j = 0; j <= static_cast<int>(BufferAccessTypePAM::kReadWrite); ++j) {
      ...
      for (int j = 0; j <= static_cast<int>(ReuseTypePAM::kNoReuse); ++j) {
    ...
    for (int i = 0; i < max_n_bufs - n_bufs; ++i) {
      for (int j = 0; j <= static_cast<int>(BufferAccessTypePAM::kReadWrite); ++j) {  // 3
        ret->push_back(0.0f);
      }
      ret->push_back(0.0f);
      ret->push_back(0.0f);
      ret->push_back(0.0f);
      ret->push_back(0.0f);
      for (int j = 0; j <= static_cast<int>(ReuseTypePAM::kNoReuse); ++j) {  // 3
        ret->push_back(0.0f);
      }
      ret->push_back(0.0f);
      ret->push_back(0.0f);
  ...
  // feature for tiling pattern 24 * (pattern_index >= 0? 8  : 0)
  if (pattern_index >= 0) {
    for (int pattern_i = 0; pattern_i < 8; pattern_i ++) {
      const auto& item = extractor.buf_seq[pattern_i + pattern_index];
    // for (const auto& item : extractor.buf_seq) {
      const BufferFeatureSetPAM& buf_fea_set = extractor.per_buffer_features[item];

      /***** Group 1: Computation related features  * 1  *****/
      ret->push_back(slog(buf_fea_set.float_op));

      /***** Group 2: Buffer access related features * 21 *****/
      const BufferAccessFeaturePAM& acc_fea = buf_fea_set.access_fea;
      for (int j = 0; j <= static_cast<int>(LocationTypePAM::localMem); ++j) {
        ret->push_back(j == static_cast<int>(acc_fea.buf_loc));
      }
      for (int j = 0; j <= static_cast<int>(BufferAccessTypePAM::kReadWrite); ++j) {
        ret->push_back(j == static_cast<int>(acc_fea.acc_type));
      }
      ...
      /***** Group 4: Allocation related features * 1*****/
      ret->push_back(slog(buf_fea_set.alloc_size));
    }
  }
}

void GetPerStoreFeaturesWorkerFuncPAM(const SearchTask& task, const State& state, int    max_n_bufs,
  ...
  try {
    ...
    // std::cout << "prim_func body\t" << prim_func->body << std::endl; 
    GetPerStoreFeaturePAM(prim_func->body, task->hardware_params->cache_line_bytes, max_n_bufs,
                       feature);
  } catch (Error& e) {
    (*error_ct)++;
  }
...
void ExtractAndPaddingFeaturePAM(const SearchTask& task, const State& state, int max_n_bufs,
                                   std::vector<std::vector<float>> *bufferseq_feas, int& fea_size, int& kmp_index, std::atomic<int>* error_ct) {

  // std::vector<std::vector<int>> step_vecs;
  std::vector<float> feature;
  
  // // std::cout << "start end" << std::endl;
  GetPerStoreFeaturesWorkerFuncPAM(task, state, max_n_bufs, &feature, error_ct);
  if (!feature.empty()) {
    size_t buf_fea_sizes = static_cast<size_t>(feature[0]);
    fea_size = static_cast<int>(feature[0]);
    kmp_index = static_cast<int>(feature[1]);
    // // std::cout << "bufferLoad size\t#" << buf_fea_sizes  << "fea_size\t" << fea_size << std::endl;
    // for (size_t i = 0; i < max_line_len && i < buf_fea_sizes; i++) {
    for (size_t i = 0; i < buf_fea_sizes; i++) {
      std::vector<float> fea;
      fea.insert(fea.begin(), feature.begin() + 2 + i * Ansor_FEATURE_SIZE, feature.begin() + 2 + (i + 1) * Ansor_FEATURE_SIZE);
      (*bufferseq_feas).push_back(fea);
    }
    if (kmp_index >= 0) {
      for (size_t i = 0; i < 8; i++) {
        std::vector<float> per_buf_fea;
        per_buf_fea.insert(per_buf_fea.begin(), feature.begin() + 2 + Ansor_FEATURE_SIZE * buf_fea_sizes + i * PAM_FEATURE_SIZE, feature.begin() + 2 + Ansor_FEATURE_SIZE * buf_fea_sizes + (i+1) * PAM_FEATURE_SIZE);
        (*bufferseq_feas).push_back(per_buf_fea);
      }
    }
  }
  // for (size_t i = bufferseq_feas->size(); i < max_line_len; i++) {
  //   std::vector<float> vec(PAM_FEATURE_SIZE, 0);
  //   bufferseq_feas->push_back(std::move(vec));
  // }
  // // std::cout << pstep_vecs->size() << "\t#" << pstep_vecs[0][0].size() << std::endl;
  // StdCout(1) << state << "\n" << pstep_vecs << std::endl;
  // assert(pstep_vecs->size() == max_line_len);
  assert(bufferseq_feas->size() == fea_size + (kmp_index >=0? 8: 0));
}
...
void GetPerStoreFeaturesFromStatesPAM(const Array<State>& states, const SearchTask& task,
                                   ...
                                   std::vector<int>* kmp_indexes)  {
  // extract features
  features->assign(states.size(), std::vector<std::vector<float>>());
  fea_sizes->assign(states.size(), 0);
  kmp_indexes->assign(states.size(), -1);

  std::atomic<int> error_ct(0);

  support::parallel_for(skip_first_n_feature_extraction, states.size(),
                        [&task, &states, &max_n_bufs, &features, &error_ct, &fea_sizes, &kmp_indexes](int i) {
                          ExtractAndPaddingFeaturePAM(task, states[i], max_n_bufs,
                                                        &(*features)[i], (*fea_sizes)[i], (*kmp_indexes)[i], &error_ct);
                        });
}
...
void GetPerStoreFeaturesFromStatesPAM(const Array<State>& states, const std::vector<SearchTask>& tasks,
  ...
  support::parallel_for(skip_first_n_feature_extraction, states.size(),
                        [&tasks, &states, &max_n_bufs, &features, &fea_sizes, &kmp_indexes, &error_ct](int i) {
                          ExtractAndPaddingFeaturePAM(tasks[i], states[i], max_n_bufs,
                                                        &(*features)[i], (*fea_sizes)[i], (*kmp_indexes)[i], &error_ct);
                        });
}
...
void GetPerStoreFeaturesFromMeasurePairsPAM(const Array<MeasureInput>& inputs,
  ...
  GetPerStoreFeaturesFromStatesPAM(states, tasks, 0, max_n_bufs, features, fea_sizes, kmp_indexes);
}
 ...
 */
TVMByteArray SerializeFeaturesPAM(std::vector<std::vector<std::vector<float>>>&& features,
                               std::vector<int>&& fea_sizes,
                               std::vector<int>&& kmp_indexes,
                               std::vector<float>&& normalized_throughputs,
                               std::vector<int>&& task_ids,
  ...
  // serialize sizes
  // feature: N * buffer_seq * Buffer_Embedding_Dim
  ...
  //   size_vector.push_back(static_cast<int>(x.size() * Ansor_FEATURE_SIZE));
  //   total_bytes += sizeof(float) * (x.size() * PAM_FEATURE_SIZE + 2);
  // }
  for (size_t i = 0; i < features.size(); i++) {
    int stmt_num = fea_sizes[i];
    int buf_num = kmp_indexes[i] >=0? 8: 0;
    size_vector.push_back(static_cast<int>(stmt_num * Ansor_FEATURE_SIZE + buf_num * PAM_FEATURE_SIZE));
    total_bytes += sizeof(float) * (stmt_num * Ansor_FEATURE_SIZE + buf_num * PAM_FEATURE_SIZE + 2);
  }
  ...
  // [N, buf_seq + buf_size, buf_seq + buf_size, ..., normalsize, tasksize, mincostsize] = 1 + n + 3
  ...
  // serialize feature_size
//   // std::cout << fea_sizes[0] << "\t" << fea_sizes[1] << std::endl;
  memmove(ptr, reinterpret_cast<char*>(fea_sizes.data()), fea_sizes.size() * sizeof(int));
  ptr += fea_sizes.size() * sizeof(int);
  memmove(ptr, reinterpret_cast<char*>(kmp_indexes.data()), kmp_indexes.size() * sizeof(int));
  ptr += kmp_indexes.size() * sizeof(int);

  // serialize features
  for (auto& x : features) {
    for (auto& y : x) {
      memmove(ptr, y.data(), sizeof(float) * y.size());
      ptr += sizeof(float) * y.size();
      y.clear();
    }
    x.clear();
  }
...
TVM_REGISTER_GLOBAL("auto_scheduler.GetPerStoreFeaturesFromStatesPAM")
    .set_body([](TVMArgs args, TVMRetValue* ret) {
      ...
      GetPerStoreFeaturesFromStatesPAM(states, task, 0, max_n_bufs, &features, &fea_sizes, &kmp_indexes);

      std::vector<char> byte_data;
      *ret = SerializeFeaturesPAM(std::move(features), std::move(fea_sizes), std::move(kmp_indexes),
            std::move(normalized_throughputs), std::move(task_ids), std::move(min_costs), &byte_data);
    });

TVM_REGISTER_GLOBAL("auto_scheduler.GetPerStoreFeaturesFromMeasurePairsPAM")
    .set_body([](TVMArgs args, TVMRetValue* ret) {
      ...
      GetPerStoreFeaturesFromMeasurePairsPAM(inputs, results, skip_first_n_feature_extraction, max_n_bufs,
                                        &features, &fea_sizes, &kmp_indexes, &normalized_throughputs,
                                          &task_ids, &min_costs);
