...
from tvm.auto_scheduler.cost_model.pam_model import PAMModelInternal
...
def train_zero_shot(dataset, train_ratio, model_names, split_scheme, use_gpu, cur_fold, load_model):
    # Split dataset
    ...
    for name in names:
        ...
        if os.path.isfile(load_model):
            print(load_model, "\tload model defined")
            model.load(load_model)
            model.few_shot_learning='fine_tune_mix_task'
        models.append(model)

    eval_results = []
    for name, model in zip(names, models):
        # Train the model
        filename = os.path.join(args.save_path, str(cur_fold), 'fine_tune_'+name + '_' + args.dataset[0].split('_')[-2] + '_{}_{}.pkl'.format(args.file_cnt, str(args.saveName)))
        print("Save model to %s" % filename)
        # exit()
        model.fit_base(train_set, valid_set=test_set, epoch=50, save_dir=os.path.join(args.save_path, str(cur_fold)))
        
        dfs_remove_weight(os.path.join(args.save_path, str(cur_fold)), retain=5)
        model.save(filename)
        # Evaluate the model
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, default='./ckpt/a100/500k')
    parser.add_argument("--load_model", type=str, default='./ckpt/k80/1500')
    parser.add_argument("--file_cnt", type=int, default=0)
    parser.add_argument("--ratio", type=float, default=1.0) # 0.15625 = 250 / 1600
    parser.add_argument("--select_cnt", type=int, default=-1) # 0.15625 = 250 / 1748
    ...
    for i in range(1, args.fold):
        ...
        for j in range(1, len(args.dataset)):
            tmp_dataset = pickle.load(open(args.dataset[j], "rb"))
            dataset.update_from_dataset(tmp_dataset)
      
        # tasks = dataset.gemmtasks()
        tasks = dataset.tasks()
        task_nums = len(tasks)
        if args.file_cnt == 0:
            args.file_cnt = task_nums
        args.saveName = int(args.file_cnt * args.ratio)
        print(args.file_cnt, task_nums, len(dataset.tasks()))

        # random select # 1
        # perm = np.random.permutation(task_nums)
        # random_select_tasks = [tasks[i] for i in perm[:args.select_cnt]]
        # print(len(random_select_tasks), 'mode:1')
        # sub_dataset = dataset.extract_subset(random_select_tasks)

        # ratio select # 2
        sub_dataset = dataset.extract_subset_ratio(args.ratio)

        del dataset
        print(len(sub_dataset.tasks()), args.saveName)
        load_model = f'{args.load_model}/pam_k80_1500.pkl'
        train_zero_shot(sub_dataset, args.train_ratio, args.models, args.split_scheme, args.use_gpu, i, load_model=load_model)
        del sub_dataset
